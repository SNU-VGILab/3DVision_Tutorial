...
To launch all 8 students at once, run `make all` (builds the image + starts `user0`–`user7`).
...
- `3DPerception.ipynb` / `3DPerception_answer.ipynb` — point cloud / voxel representations,
  SparseConvNet (WarpConvNet), 3D attention. Exercise: point cloud semantic segmentation with
  SparseConvNet & PointTransformerV3.
